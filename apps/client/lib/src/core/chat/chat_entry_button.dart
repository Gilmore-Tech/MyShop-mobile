...
class ChatEntryButton extends ConsumerStatefulWidget {
  ...
  /// Button label, e.g. `"Chat with Kwame"`.
  final String label;
...
class _ChatEntryButtonState extends ConsumerState<ChatEntryButton> {
  ...
  void _ensureOpenedAndSubscribed() {
    ...
    // Defer the openChannel call out of the build cycle.
    scheduleMicrotask(() {
      if (!mounted) return;
  ...
  Widget build(BuildContext context) {
    // Watch so the controller becomes available right after auth/SP
    // initialization without us needing a manual rebuild.
