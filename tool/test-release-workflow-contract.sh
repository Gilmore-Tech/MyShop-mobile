...
env \
  ...
  bash "$VERIFIER" "$REVIEWED_SOURCE" "$REVIEWED_SOURCE" 40 both "$REPO" >/dev/null
...
expect_failure "operator source mismatch" env \
  ...
  GITHUB_REF=refs/heads/main \
  GITHUB_SHA="$REVIEWED_SOURCE" \
  bash "$VERIFIER" "$REVIEWED_SOURCE" 0000000000000000000000000000000000000000 40 both "$REPO"
expect_failure "invalid app selection" env \
  GITHUB_EVENT_NAME=workflow_dispatch \
  GITHUB_REF=refs/heads/main \
  GITHUB_SHA="$REVIEWED_SOURCE" \
  bash "$VERIFIER" "$REVIEWED_SOURCE" "$REVIEWED_SOURCE" 40 artisan "$REPO"
