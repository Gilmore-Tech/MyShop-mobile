...
env \
  ...
  bash "$VERIFIER" "$REVIEWED_SOURCE" "$REVIEWED_SOURCE" 41 both "$REPO" >/dev/null

expect_failure "non-dispatch event" env \
  GITHUB_EVENT_NAME=push \
  GITHUB_REF=refs/heads/main \
  GITHUB_SHA="$REVIEWED_SOURCE" \
  bash "$VERIFIER" "$REVIEWED_SOURCE" "$REVIEWED_SOURCE" 41 both "$REPO"
...
expect_failure "operator source mismatch" env \
  ...
  GITHUB_REF=refs/heads/main \
  GITHUB_SHA="$REVIEWED_SOURCE" \
  bash "$VERIFIER" "$REVIEWED_SOURCE" 0000000000000000000000000000000000000000 41 both "$REPO"
expect_failure "invalid app selection" env \
  GITHUB_EVENT_NAME=workflow_dispatch \
  GITHUB_REF=refs/heads/main \
  GITHUB_SHA="$REVIEWED_SOURCE" \
  bash "$VERIFIER" "$REVIEWED_SOURCE" "$REVIEWED_SOURCE" 41 artisan "$REPO"
expect_failure "occupied build number" env \
  GITHUB_EVENT_NAME=workflow_dispatch \
  GITHUB_REF=refs/heads/main \
  GITHUB_SHA="$REVIEWED_SOURCE" \
  bash "$VERIFIER" "$REVIEWED_SOURCE" "$REVIEWED_SOURCE" 40 both "$REPO"
