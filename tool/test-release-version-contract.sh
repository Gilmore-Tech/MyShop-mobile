...
expect_failure "previous client build number" env RELEASE_BUILD_NUMBER=37 bash "$RESOLVER" client
expect_failure "previous provider build number" env RELEASE_BUILD_NUMBER=37 bash "$RESOLVER" provider
...
expect_failure "unknown app" env RELEASE_BUILD_NUMBER=38 bash "$RESOLVER" artisan

EXPECTED=$'--build-name=1.4.5\n--build-number=38\n--dart-define=MYSHOP_MARKETING_VERSION=1.4.5'
expect_output "client release" "$EXPECTED" env RELEASE_BUILD_NUMBER=38 bash "$RESOLVER" client
expect_output "provider release" "$EXPECTED" env RELEASE_BUILD_NUMBER=38 bash "$RESOLVER" provider
