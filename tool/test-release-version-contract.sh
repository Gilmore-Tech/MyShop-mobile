...
expect_failure "old build number" env RELEASE_BUILD_NUMBER=24 bash "$RESOLVER" client
expect_failure "non-portable build number" env RELEASE_BUILD_NUMBER=2100000001 bash "$RESOLVER" client
expect_failure "unknown app" env RELEASE_BUILD_NUMBER=25 bash "$RESOLVER" artisan

EXPECTED=$'--build-name=1.4.1\n--build-number=25\n--dart-define=MYSHOP_MARKETING_VERSION=1.4.1'
expect_output "client release" "$EXPECTED" env RELEASE_BUILD_NUMBER=25 bash "$RESOLVER" client
expect_output "provider release" "$EXPECTED" env RELEASE_BUILD_NUMBER=25 bash "$RESOLVER" provider
