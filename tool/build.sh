...
case "$PLATFORM" in
  ...
  ios)
    ...
    echo "  Exports via $REPO_ROOT/ExportOptions.plist with the selected build"
    echo "  number preserved and available archive symbols included."
    echo "  Output IPA in build/ios/ipa/ — upload with Transporter."

    # Xcode's export pipeline invokes Apple's /usr/bin/rsync and then starts a
    # second `rsync` process through PATH. If a MacPorts/Homebrew rsync appears
    # first, Apple's `-E` (extended attributes) is forwarded to an incompatible
    # implementation and export fails with the unhelpful `Copy failed`.
    # Preserve the operator's remaining PATH for Flutter and other tools, but
    # make Xcode's nested rsync resolve to the matching Apple implementation.
    export PATH="/usr/bin:/bin:/usr/sbin:/sbin:$PATH"
    flutter build ipa --release \
      --export-options-plist "$REPO_ROOT/ExportOptions.plist" \
      "${RELEASE_VERSION_ARGS[@]}" "${DEFINES[@]}" "$@"
    bash "$REPO_ROOT/tool/verify-ios-archive-symbols.sh" \
      "$APP_DIR/build/ios/archive/Runner.xcarchive"
