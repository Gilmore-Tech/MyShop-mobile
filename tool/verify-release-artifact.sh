...
# Verify the metadata and provenance embedded in one exact signed store artifact.
#
#   tool/verify-release-artifact.sh client android path/to/app.aab 1.4.1 24 <40-char-main-sha>
#   tool/verify-release-artifact.sh provider ios path/to/app.ipa 1.4.1 24 <40-char-main-sha>
#
# This does not upload or modify an artifact. It deliberately verifies the
# package contents rather than trusting a filename, directory or build time.
...
case "$APP" in
  client)
    ...
    # Anchored to the upload certificate on the already signed +23 candidate.
    # Play Console must still independently confirm that this is its current
    # accepted upload certificate before a final release is authorized.
...
verify_android() {
  ...
  case "$extension" in
    apk)
      ...
      actual_cert_sha1=$("$signer" verify --print-certs "$ARTIFACT" |
        awk -F': ' '/certificate SHA-1 digest:/ { print toupper($2); exit }' |
        sed 's/../&:/g; s/:$//')
      ;;
    aab)
      command -v jarsigner >/dev/null 2>&1 && command -v keytool >/dev/null 2>&1 || {
      ...
      # AAB manifests use the protobuf XML form. The audited provenance values
      # and their globally unique keys remain literal strings in that form.
      ...
      require_text "$manifest" "com.gilmoretechnologies.myshop.BUILD_NUMBER" "build-number provenance key"
      require_text "$manifest" "$EXPECTED_BUILD" "Android build-number provenance value"
      jarsigner -verify "$ARTIFACT" >/dev/null
      actual_cert_sha1=$(keytool -printcert -jarfile "$ARTIFACT" |
  ...
  require_text "$manifest" "com.gilmoretechnologies.myshop.SOURCE_COMMIT" "source provenance key"
  require_text "$manifest" "$EXPECTED_SOURCE" "reviewed source SHA"
  require_text "$manifest" "com.gilmoretechnologies.myshop.MARKETING_VERSION" "marketing-version provenance key"
  require_text "$manifest" "com.gilmoretechnologies.myshop.RELEASE_IDENTITY" "release-identity key"
  require_text "$manifest" "$EXPECTED_RELEASE_IDENTITY" "combined release identity"
  actual_cert_sha1=$(printf '%s' "$actual_cert_sha1" | tr '[:lower:]' '[:upper:]')
  [[ "$actual_cert_sha1" == "$EXPECTED_ANDROID_UPLOAD_CERT_SHA1" ]] || {
    echo "error: Android artifact signer SHA-1 does not match the reviewed app signing identity" >&2
    exit 1
  }
...
verify_ios() {
  ...
  unzip -qq "$ARTIFACT" -d "$TMP_ROOT/package"
  local app_bundle
  app_bundle=$(find "$TMP_ROOT/package/Payload" -maxdepth 1 -type d -name '*.app' -print | head -n 1)
  ...
  [[ "$(plutil -extract MyShopReleaseIdentity raw -o - "$app_bundle/Info.plist" 2>/dev/null || true)" == "$EXPECTED_RELEASE_IDENTITY" ]] || {
    echo "error: iOS combined release identity does not match $EXPECTED_RELEASE_IDENTITY" >&2
    exit 1
  }
  ...
  [[ -f "$app_bundle/embedded.mobileprovision" && -d "$app_bundle/_CodeSignature" ]] || {
