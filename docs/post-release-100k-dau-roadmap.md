...
- [ ] Configure external metrics, alerts, log retention/redaction, paging and
      ...
      `0b1c46a` adds explicit opt-in Blueprint variables plus 14
      ...
      Official Prometheus 3.12.0 validates all 14 rules; focused contracts pass
      40/40 and pinned Node 22 metrics tests pass 16/16. This does not configure
      a collector, recipient, provider dashboard, retention or production
      alert. Per-worker backlog/stale-success rules still require the approved
      workload model and runtime backlog gauges or a reviewed database exporter.
...
The fail-closed load-harness contract currently passes **18/18** local and
...
authority, scenario sizing, explicit routing isolation, inherited-secret
stripping and private external artifact enforcement. The end-to-end runner
contract injects database, Redis, JWT, k6-cloud and Maps credentials into its
parent environment and proves k6 receives none of them. The guarded
